...
import { IFields, IFiles, IPostProgress, IForm, IPost } from "@root/types/post";
...
export const parseForm = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    form.on(
      "progress",
      (
        bytesReceived: IPostProgress["bytesReceived"],
        bytesExpected: IPostProgress["bytesReceived"]
      ) => {
    ...
    form.parse(req, (err, fields: IFields, files: IFiles): void => {
      ...
      const {
        image1,
        image2,
      }: { image1: IPost["image1"]; image2: IPost["image2"] } = _.pick(files, [
        "image1.filepath",
